...
Using the **started_at** and **ended_at** columns, in YYYY-MM-DD HH:mm:ss UTC format, I created a new column named **ride_duration_minutes** (integer) to display the total duration of each ride in minutes. I will also create new columns **day_of_week** (string), **month** (string), and **hour** (string) to help with analysis later on. As we create the new dataset, we can drop the **ride_id**, **start_station_name**, **start_station_id**, **end_station_name**, **end_station_id**, **start_lat**, **start_lng**, **end_lat** and **end_lng** columns as they serve no value to our analysis and keep the dataset cleaner. With these changes, a total of **269,711 rows** were deleted.
